...
function qcf_form_options () {
	...
	$content .= '
		...
		<h2>Allow attachments</h2>
		<p><input type="checkbox" style="margin: 0; padding: 0; border: none;" name="qcf_attach"' . $qcf['qcf_attach'] . ' value="checked" /> User can attach files</p>
		<p>Field Label</p>
		<p><input type="text" style="width:100%;" name="qcf_attach_label" value="' . $qcf['qcf_attach_label'] . '" /></p>
		<p>Maximum File size</p>
		<p><input type="text" style="width:100%;" name="qcf_attach_size" value="' . $qcf['qcf_attach_size'] . '" /></p>
		<p>Allowable file types</p>
		<p><input type="text" style="width:100%;" name="qcf_attach_type" value="' . $qcf['qcf_attach_type'] . '" /></p>
...
function qcf_get_default_options () {
	...
	$qcf['qcf_attach'] = '';
	$qcf['qcf_attach_label'] = 'Attach a file:';
	$qcf['qcf_attach_size'] = '100000';
	$qcf['qcf_attach_type'] = 'jpg,gif,png,pdf,doc,odf,xls';
	$qcf['send'] = 'Send it!';
	$qcf['captcha'] = '';
...
function qcf_verify_form(&$values, &$errors)
	...
	if (file_exists($tmp_name)) {
		if ($size > $qcf['qcf_attach_size']) $errors['attach'] = '<p class="error">' . $error['attach_size'] . '</p>'; 
		$ext = substr(strrchr($name,'.'),1);
		$pos = strpos($qcf['qcf_attach_type'],$ext);
		if (strpos($qcf['qcf_attach_type'],$ext) === false) $errors['attach'] = '<p class="error">' . $error['attach_type'] . '/' . $name . '/' . $ext . '/' . $qcf['qcf_attach_type'] . '/' . $pos . '/</p>'; 
		}
...
function qcf_display_form( $values, $errors, $whichpage )
	...
	if ($qcf['qcf_attach'] == "checked") {
		if ($errors['attach']) $content .= $errors['attach'];
		else $content .= '<p class="input">' . $qcf['qcf_attach_label'] . '</p>';
		$content .= '<p><input type="file" size="10" name="filename"></p>';
		}
	if ($qcf['captcha'] == "checked")
		{
